...
if [ ${#AUDIOS[@]} -gt 0 ]; then
    ...
    magick -size "${MAX_TEXT_WIDTH}x${MAX_TEXT_HEIGHT}" -background none -font "${FONT_PATH}" -pointsize ${FONTSIZE} -gravity East -fill white caption:@"${TEXT_FILE}" -trim +repage "${TEMP_PNG}"
